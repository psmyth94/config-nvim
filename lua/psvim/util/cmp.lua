...
-- License: Apache-2.0
---@class psvim.util.cmp
local M = {}

---@alias psvim.util.cmp.Action fun():boolean?
---@type table<string, psvim.util.cmp.Action>
M.actions = {
  -- Native Snippets
...
function M.setup(opts)
  ...
  require('cmp.utils.snippet').parse = function(input)
    ...
    return PSVim.cmp.snippet_preview(input)
  end
  ...
  cmp.event:on('confirm_done', function(event)
    if vim.tbl_contains(opts.auto_brackets or {}, vim.bo.filetype) then
      PSVim.cmp.auto_brackets(event.entry)
    end
  end)
  cmp.event:on('menu_opened', function(event)
    PSVim.cmp.add_missing_snippet_docs(event.window)
  end)
end
