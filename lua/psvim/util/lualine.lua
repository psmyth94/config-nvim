...
function M.cmp_source(name, icon)
  icon = icon or PSVim.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
  local started = false
...
function M.pretty_path(opts)
  ...
  return function(self)
    ...
    local root = PSVim.root.get({ normalize = true })
    local cwd = PSVim.root.cwd()
